...
async function carregarTransacao() {
    ...
    if (selectedOption) {
        ...
        historicoTransacao.forEach(transacao => {
            ...
            valor.textContent = `R$ ${transacao.valor.toFixed(2)}`; // Format and set value cell content
...
async function deletarUsuario() {
    ...
    if (selectedOption) {
      ...
      await callBack({
        metodo: "DELETE",
        url: `http://localhost:3000/usuarios/delete/${optionValue}`});
...
async function atualizarBarraFelicidade() {
    ...
    if (selectedOption) {
        ...
        await callBack({
            metodo: "PATCH",
            url: `http://localhost:3000/usuarios/patch/${selectedOptionValue}`,
...
async function adicionarTransacao(descricao, valor) {
    ...
    colunaValor.textContent = `R$ ${valor.toFixed(2)}`; // Format and set value cell content
